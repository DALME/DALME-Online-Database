...
class Stage(BaseStage):
    ...
    def adjust_id_columns(self):
        ...
        app_labels = [
            ('dalme', 'taggit', None),
            ('dalme', 'wagtailadmin', None),
            ('dalme', 'wagtailcore', None),
            ('dalme', 'wagtaildocs', None),
            ('dalme', 'wagtailembeds', None),
            ('dalme', 'wagtailforms', None),
            ('dalme', 'wagtailimages', None),
            ('dalme', 'wagtailredirects', None),
            ('public', 'wagtailusers', None),
            ('dalme', 'web', None),
            ('dalme', 'webimages', None),
            ('public', 'domain', ['localereference', 'languagereference', 'countryreference', 'workflow', 'worklog']),
        ]

        for schema, app, models in app_labels:
            self.logger.info('Adjusting autogenerated id sequences for "%s" models', app)
            app_config = apps.get_app_config(app)
            if app_config.models_module is not None:
                if models is None:
                    models = [m.__name__.lower() for m in app_config.get_models()]  # noqa: PLW2901

                for model in models:
                    qualified_name = f'{app}_{model}'
