...
class JoinField(fields.DEDField, dsl_Join):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self._public = public
        kwargs['relations'] = {'WebRecord': 'WebFolio'} if public else {'FullRecord': 'FullFolio'}
    ...
    def get_value_from_instance(self, instance, field_value_to_ignore=None):  # noqa: ARG002
        ...
        if isinstance(instance, Record):
            return 'WebRecord' if self._public else 'FullRecord'

        if isinstance(instance, PageNode):
            return {
                'name': 'WebFolio' if self._public else 'FullFolio',
...
web_records = Index('web_records')
web_records.mapping(default_mapping)
web_records.settings(analysis=analysis)
full_records = web_records.clone('full_records')


@web_records.document
class WebRecordBase(Document):
    """Base web record document."""
...
@registry.register_document
class FullRecord(FullRecordBase, WebRecord):
    """Record document."""
...
@registry.register_document
class FullFolio(FullRecordBase, WebFolio):
    """Folio document."""
