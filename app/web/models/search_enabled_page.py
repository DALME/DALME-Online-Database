...
class SearchEnabled(RoutablePageMixin, BasePage):
    ...
    def search(self, request, pk=None):
        ...
        if request.method != 'POST' and request.session.get('web-search-post', False):
            seconds = 86401
            ...
            stored_dt = datetime.fromtimestamp(
                request.session.get('web-search-ts', default_ts),
                tz=timezone.get_current_timezone(),
            )
            delta = datetime.now(tz=timezone.get_current_timezone()) - stored_dt
            if delta.seconds < seconds:
                request.POST = request.session['web-search-post']
                request.method = 'POST'

        if request.method == 'POST':
            formset = search_formset(request.POST, form_kwargs={'fields': search_context.fields})
            request.session['web-search-post'] = request.POST
            request.session['web-search-ts'] = datetime.timestamp(
        ...
        context.update(
            {
                ...
                'records': True,
                'browse_mode': request.session.get('web-browse-mode', 'wide'),
            },
        )
        ...
        context.update(
            {
                ...
                'viewer_mode': request.session.get('web-viewer-mode', 'vertical-split'),
                'render_mode': request.session.get('web-render-mode', 'scholarly'),
                'purl': purl,
