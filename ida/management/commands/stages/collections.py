...
from ida.models import Attribute, AttributeType, Permission, Record, Tenant, User
...
class Stage(BaseStage):
    ...
    def migrate_collection(self):
        ...
        https://github.com/ocp/DALME-Online-Database/blob/bc4ff5979e14d14c8cd8a9a9d2f1052512c5388d/dalme_app/migrations/0007_data_m_collections.py#L37

        """
        if Collection.objects.count() == 0:
            user_obj = User.objects.get(pk=1)
            collection_ct = ContentType.objects.get(app_label='dalme_app', model='collection')
            ...
            with connection.cursor() as cursor:
                ...
                cursor.execute('SELECT * FROM restore.dalme_app_set;')
                ...
                for row in rows:
                    ...
                    if stat_title is not None and stat_text is not None:
                        Attribute.objects.create(
                            ...
                            value_json={stat_title: stat_text},
                            creation_user=user_obj,
                            modification_user=user_obj,
                        )

                    # Workset tracking.
                    cursor.execute('SELECT * FROM restore.dalme_app_set_x_content WHERE set_id_id = %s;', [row_id])
                    members = self.map_rows(cursor)

                    if set_type == WORKSET_TYPE:
                        workset_progress = [member['object_id'] for member in members if member['workset_done']]
                        Attribute.objects.create(
                            content_type=collection_ct,
                            object_id=row_id,
                    ...
                    # https://github.com/ocp/DALME-Online-Database/blob/bc4ff5979e14d14c8cd8a9a9d2f1052512c5388d/dalme_app/migrations/0007_data_m_collections.py#L159
