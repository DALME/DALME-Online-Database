...
def get_script_menu_item(name=None, description=None, type=None):
    ...
    currentItem = (
        '<a class="script-item d-flex text-dark-gray" href="/tools/scripts?s={}"><div class="script-icon \
                   bg-{}-soft"><i class="fas {} text-{}"></i></div>'.format(
            name, type, icon_dict[type], type
        )
    )
    currentItem += '<span class="font-weight-bold mr-1">{}: </span> {}</a>'.format(
        name, description
    )
    return currentItem
...
def create_json_field_reps(request):
    with open(
        os.path.join("dalme_app", "config", "datatables", "field_defs", "sources.json")
    ) as f:
        input = json.load(f)
    for field in input:
        with open(
...
def test_django_q(request):
    async_task(
        "dalme_app.tasks.test_task",
        request.user.id,
        task_name="test_task",
        hook="dalme_app.utils.send_message",
    )


def update_folios_in_dam(request):
    async_task(
        "dalme_app.tasks.update_rs_folio_field",
        request.user.id,
        task_name="update_folios_in_dam",
        hook="dalme_app.utils.send_message",
    )
    ...
    with open(file, "r") as fp:
        ...
        for item in text:
            if not Language.objects.filter(
                glottolog_id=item["properties"]["language"]["id"]
            ).exists():
                new_lang = Language()
                ...
                if item["properties"]["language"]["newick"]:
                    ...
                    if m:
                        parent_object = Language.objects.get(
                            glottolog_id=item["properties"]["language"]["id"]
                        )
                        for i in m:
...
def migrate_datasets(request):
    ...
    datasets = list(
        set([i.value_STR for i in Attribute.objects.filter(attribute_type=27)])
    )
    ...
    for s in sources:
        ...
        if not Set_x_content.objects.filter(
            set_id=set_object.id, object_id=s.object_id
        ).exists():
            new_entry = Set_x_content(set_id=set_object, content_object=source_object)
...
def add_to_public_register(request):
    ...
    for source in public_sources:
        PublicRegister.objects.get_or_create(object_id=source.id, content_type=ct)
    return "done"
    ...
    for loc in locales:
        try:
            location = geolocator.geocode(
                f"{loc.name}, {loc.administrative_region}, {loc.country.name}"
            )
        ...
        if inv.pages:
            for fol in inv.pages.all():
                if fol.sources.first().transcription:
                    ...
                    tr = Transcription.objects.filter(pk=tr_id).update(
                        transcription=text
                    )
    return "cool"
    ...
    for e in data:
        ...
        if e[3] == 1:
            source_object.tags.create(
                tag_type="C", tag="Done", tag_group="DLS_Lucca_Transcription_Review"
            )
