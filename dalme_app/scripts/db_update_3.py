...
def step_2():
    ...
    try:
        for entry in _users:
            ...
            user = User.objects.create_user(**entry)
            user.save()
...
def step_3():
    ownership = {
        ...
        'Bologna': 36,
        'Bonifacio': 5,
        # 'Hérault': User.objects.get(username='laumo').id,
        'Najac': User.objects.get(username='guilhem.ferrand').id,
        'València': User.objects.get(username='anbegon').id,
        }
    ...
    for k, v in ownership.items():
        ...
        for source in sources:
            ...
            obj.owner = User.objects.get(pk=v)
            obj.save()
    ...
    for source in sources:
        source.owner = User.objects.get(pk=65)
    ...
    for source in sources:
        source.owner = User.objects.get(pk=37)
...
def step_4():
    ...
    t_set.permissions = 4
    t_set.owner = User.objects.get(pk=5)
    ...
    for source in sources:
        print(source.id)
        source.owner = User.objects.get(pk=36)
