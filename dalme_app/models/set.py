...
from dalme_app.models.templates import dalmeBasic, dalmeUuid, dalmeOwned
...
class Set(dalmeUuid, dalmeOwned):
    ...
    dataset_usergroup = models.ForeignKey(Group, on_delete=models.CASCADE, related_name='dataset', limit_choices_to={
                                          'properties__type': 3}, null=True)
    ...
    def detail_string(self):
        ...
        result = result + ' | {}'.format(self.dataset_usergroup.name) if self.set_type == 3 else result + \
            ' | {}'.format(self.owner.profile.full_name)
    ...
    def get_time_coverage(self):
        years = self.members.filter(source__attributes__attribute_type__in=[19, 25, 26]).order_by(
            'source__attributes__value_DATE_y').values_list('source__attributes__value_DATE_y', flat=True)
        return dict(Counter(years))

    def get_public_time_coverage(self):
        years = self.members.filter(source__attributes__attribute_type__in=[19, 25, 26], source__workflow__is_public=True).order_by(
            'source__attributes__value_DATE_y').values_list('source__attributes__value_DATE_y', flat=True)
        return dict(Counter(years))
