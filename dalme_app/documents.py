...
from dalme_app.models import LocaleReference, Source, Source_pages
...
class JoinField(fields.DEDField, dsl_Join):
    def __init__(self, multi=False, required=False, *args, **kwargs):
        ...
        self._public = public
        kwargs["relations"] = (
            {"PublicSource": "PublicFolio"} if public else {"FullSource": "FullFolio"}
        )
...
dynamic_template = {
    "standard_string_fields": {
        ...
        "mapping": {"type": "keyword", "normaliser": basic_normalizer},
    }
}
...
class PublicSource(PublicSourceBase):
    ...
    def update(self, thing, refresh=None, action="index", parallel=False, **kwargs):
        if (
            isinstance(thing, models.Model)
            and not thing.is_public
            and action == "index"
        ):
            action = "delete"
    ...
    def prepare_credits(self, instance):
        ...
        for credit in instance.credits.all():
            ...
            credit_list.append({"agent": f"{agent} ({type})", "type": type})
        return credit_list
...
class PublicFolio(PublicSourceBase):
    ...
    def get_queryset(self):
        return Source_pages.objects.filter(
            source__type=13, source__workflow__is_public=True
        )
    ...
    def prepare_transcription(self, instance):
        ...
        try:
            try:
                tree = et.fromstring(
                    "<xml>" + instance.transcription.transcription + "</xml>",
                    xml_parser,
                )
                return et.tostring(
                    tree, encoding="unicode", xml_declaration=False, method="text"
                )
