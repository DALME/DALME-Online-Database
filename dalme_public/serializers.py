...
from dalme_app.models import Attribute, Set, Set_x_content, Source
...
class PublicCollectionMembershipSerializer(serializers.ModelSerializer):
    ...
    name = serializers.CharField(
        source="set_id.name", read_only=True, required=False, max_length=255
    )
...
class PublicSourceSerializer(serializers.ModelSerializer):
    ...
    parent = serializers.PrimaryKeyRelatedField(
        queryset=Source.objects.all(), allow_null=True
    )
    parent_name = serializers.StringRelatedField(
        source="parent", read_only=True, required=False
    )
    ...
    collections = PublicCollectionMembershipSerializer(
        source="sets", many=True, required=False
    )
    ...
    def to_representation(self, instance):
        ...
        ret["archival_location"], ret["format_info"], ret["parent_type"] = (
            self.get_archival_info(instance)
        )
    ...
    def process_attributes(self, qset):
        ...
        if dates:
            if "start_date" in dates and "end_date" in dates:
                result["date"] = FormatDalmeDate(
                    [dates["start_date"], dates["end_date"]]
                ).format("short", "month_year")
                result["full_date"] = FormatDalmeDate(
                    [dates["start_date"], dates["end_date"]]
                ).format("long", "full")
            elif "date" in dates:
                result["date"] = FormatDalmeDate(dates["date"]).format(
                    "short", "month_year"
                )
                result["full_date"] = FormatDalmeDate(dates["date"]).format(
                    "long", "full"
                )
            else:
                result["date"] = FormatDalmeDate(list(dates.values())[0]).format(
                    "short", "month_year"
                )
                result["full_date"] = FormatDalmeDate(list(dates.values())[0]).format(
                    "long", "full"
                )
        else:
            result["date"] = "—"
        ...
        if languages:
            result["language"] = ", ".join(languages)
        ...
        if _parent.parent:  # register
            try:
                format_values = {
                    ...
                    ).values("attribute_type__short_name", "value_STR")
                }
            ...
            archive_url = _parent.parent.attributes.filter(
                attribute_type__short_name="url"
            )
            archive_url = (
                archive_url.first().value_STR if archive_url.exists() else None
            )
            marks = (
                loc.replace(archive_name, "").strip() if archive_name in loc else None
            )
            marks = marks[1:] if marks and marks.startswith(",") else marks
            if archive_name and archive_url and marks:
                return (
                    f'<a href="{archive_url}" target="_blank">{archive_name}</a>, {marks}',
                    format,
                    "register",
                )
            else:
            ...
            zotero_key = _parent.attributes.filter(
                attribute_type__short_name="zotero_key"
            )
