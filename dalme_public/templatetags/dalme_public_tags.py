...
def render_image(record):
    ...
    img = image.get_rendition(f'{resize_rule}-{dimensions}') if resize_rule and resize_rule != 'background' else image.get_rendition('original')
    ...
    if resize_rule == 'background':
        ...
        width = width if int(width) < 308 else 308
