    ...
    },
    {
        ...
        'body': [
            (
                'text',
                RichText(
                    '<p>Your privacy and the security of your data is important to us. To better protect your privacy, we provide this notice explaining our online information practices and the choices you can make about the way your information is collected and used. To make this notice easy to find, we make it available on our homepage and at every point where personally identifiable information may be requested. This notice applies to all information collected or submitted on the DALME website.</p>'
                ),
            ),
            ...
            (
                'text',
                RichText(
                    '<p>On some pages, you can make requests and register to receive materials. The types of personal information collected on these pages are name and email address. On some pages, you can submit information to other people. For example, if you want to forward an article to another person, you will need to submit the recipient’s email address.</p>'
                ),
            ),
...
MARSEILLE_COLLECTION = {
    ...
    'body': [
        ...
        (
            'text',
            RichText(
                '<p>Vestibulum vitae sagittis purus. Sed aliquet tincidunt neque. Donec vitae leo imperdiet, vehicula ligula gravida, sollicitudin ligula. Duis sit amet nulla neque. In hac habitasse platea dictumst. Vivamus tortor metus, faucibus a pulvinar non, ullamcorper non leo. Morbi a aliquam turpis. Duis ac eleifend nunc. Nam imperdiet velit ac enim venenatis, vel porta metus elementum. Vestibulum viverra placerat accumsan. Orci varius natoque penatibus et magnis dis parturient montes, nascetur ridiculus mus.</p>'
            ),
        ),
        ...
        (
            'text',
            RichText(
                '<p>Proin sollicitudin neque dignissim mauris luctus cursus. Maecenas vitae augue posuere, mattis elit id, placerat magna. Fusce laoreet quam non justo viverra, nec consequat augue fermentum. Nam mattis, tellus at pulvinar condimentum, libero libero ultrices velit, nec euismod dui felis in ligula. Morbi mollis augue eget tempus dignissim. Sed aliquam odio at interdum finibus. Curabitur consequat hendrerit libero non lobortis. Donec ut leo turpis. Vivamus convallis fringilla nisl nec tristique. Nulla congue, urna ut accumsan varius, sapien augue tristique orci, id rutrum orci est in justo.</p>'
            ),
        ),
...
class Command(BaseCommand):
    ...
    def create_corpora(self):
        ...
        Corpus.objects.create(collections=[marseille], page_id=self.collections.pk, **CORPUS_DATA)
    ...
    def create_collections_page(self):
        self.collections = self.home.add_child(instance=Collections(**COLLECTIONS_DATA))

    def create_feature_pages(self):
        source_id = FEATURED_OBJECT_DATA.pop('source_id')
        instance = FeaturedObject(source=Source.objects.get(pk=source_id), **FEATURED_OBJECT_DATA)
        self.features.add_child(instance=instance)

        source_id = FEATURED_INVENTORY_DATA.pop('source_id')
        instance = FeaturedInventory(source=Source.objects.get(pk=source_id), **FEATURED_INVENTORY_DATA)
        self.features.add_child(instance=instance)
    ...
    def create_home(self):
        ...
        Site.objects.create(hostname='localhost', site_name='DALME', root_page=root, is_default_site=True)
        return home
