...
class Command(BaseCommand):
    help = 'Bootstrap the DALME public site CMS tree.'  # noqa: A003
    ...
    def create_corpora(self):
        ...
        Corpus.objects.create(
            collections=[marseille],
            page_id=self.collections.pk,
            **CORPUS_DATA,
        )
    ...
    def create_feature_pages(self):
        ...
        instance = FeaturedObject(
            source=Record.objects.get(pk=source_id),
            **FEATURED_OBJECT_DATA,
        )
    ...
    def handle(self, *args, **options):  # noqa: ARG002
        with schema_context('dalme'):
            if (env := os.environ.get('ENV')) == 'production':
                logger.error('This command should never be run in production')
            elif env == 'staging' and Home.objects.exists():
                logger.info('Skipping site generation as the CMS tree already exists')
            else:
