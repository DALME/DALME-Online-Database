...
from wagtail.admin.views.generic.chooser import ChooseView, ChosenResponseMixin, ChosenViewMixin
...
# from .forms import ReferenceLinkChooserForm
...
class BiblioChooserViewSet(ChooserViewSet):
    ...
    def get_block_class(self, name=None, module_path=None):
        meta = type(
            ...
            {
                'icon': self.icon,
                'template': 'public/blocks/bibliography.html',
            },
        )
        cls = type(
            name,
...
class ReferenceQuerySet(APIQuerySet):
    ...
    def get_filters_as_query_dict(self):
        ...
        for key, val in self.filters:
            ...
            key = self.filter_field_aliases.get(key, key)  # noqa: PLW2901
