...
from ida.utils import IDADate
from public.serializers.attribute import AttributeSerializer
from public.serializers.collection_membership import CollectionMembershipSerializer
...
class RecordSerializer(serializers.ModelSerializer):
    ...
    collections = CollectionMembershipSerializer(many=True, required=False)
    ...
    def process_attributes(qset):
        ...
        if dates:
            if 'start_date' in dates and 'end_date' in dates:
                result['date'] = IDADate([dates['start_date'], dates['end_date']]).format_as('mys')
                result['full_date'] = IDADate([dates['start_date'], dates['end_date']]).format_as('dmyl')
            elif 'date' in dates:
                result['date'] = IDADate(dates['date']).format_as('mys')
                result['full_date'] = IDADate(dates['date']).format_as('dmyl')
            else:
                result['date'] = IDADate(next(iter(dates.values()))).format_as('mys')
                result['full_date'] = IDADate(next(iter(dates.values()))).format_as('dmyl')
        else:
            result['date'] = '—'
